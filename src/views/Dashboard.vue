<template>
  ...
  <div class="heading">Welcome , <span style="font-weight:600">{{ uname }}</span> </div>
...
<script>
import {getQuizList,getAvailableQuiz,getQuizListByID} from "../data/data.js"
...
async created(){
  const filteredIds = await getAvailableQuiz()
   if(!!filteredIds){
   axios.all(filteredIds.map(l => getQuizListByID(l)))
